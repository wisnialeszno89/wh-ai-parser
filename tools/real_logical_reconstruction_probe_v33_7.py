...
def analyze_member_relationships(profile):
    ...
    for i in range(len(members)):
        ...
        ax1 = a.rect.x
        ay1 = a.rect.y
        ax2 = a.rect.x + a.rect.width
        ay2 = a.rect.y + a.rect.height
        ...
        for j in range(i + 1, len(members)):
            ...
            bx1 = b.rect.x
            by1 = b.rect.y
            bx2 = b.rect.x + b.rect.width
            by2 = b.rect.y + b.rect.height
            ...
            area_a = a.rect.width * a.rect.height
            area_b = b.rect.width * b.rect.height

            union = area_a + area_b - intersection

            iou = intersection / union if union > 0 else 0.0

            center_ax = a.rect.x + a.rect.width / 2
            center_ay = a.rect.y + a.rect.height / 2
            center_bx = b.rect.x + b.rect.width / 2
            center_by = b.rect.y + b.rect.height / 2

            center_distance = (
                (center_ax - center_bx) ** 2
                + (center_ay - center_by) ** 2
            ) ** 0.5
            ...
            relationships.append({
                ...
                "contains": contains_a_b or contains_b_a,
                "contains_a_b": contains_a_b,
                "contains_b_a": contains_b_a,
...
def classify_member_relationships(relationships):
    evidence = []
    ...
    for relation in relationships:
        ...
        if contains and iou >= 0.70:
            evidence.append("CONTAINS_HIGH_OVERLAP")

        elif iou >= 0.80 and area_ratio >= 0.70:
            evidence.append("NEAR_DUPLICATE_GEOMETRY")

        elif iou >= 0.70:
            evidence.append("STRONG_OVERLAP")

        elif iou >= 0.50:
            evidence.append("MODERATE_OVERLAP")

        elif iou >= 0.15:
            evidence.append("PARTIAL_OVERLAP")

        else:
            evidence.append("LOW_OVERLAP")

    return evidence


def print_member_relationship_report(evaluated):
    print()
    print("-" * 100)
    print("MEMBER RELATIONSHIP REPORT")
    print("-" * 100)
    ...
    for item in evaluated[:TOP_RESULTS]:
        ...
        relationships = analyze_member_relationships(profile)
        evidence = classify_member_relationships(relationships)
        ...
        print(
            f"{item.object_id} | "
            f"members={len(profile.members)} | "
            f"relations={len(relationships)}"
        )

        for index, relation in enumerate(relationships):
            print(
                f"  pair={relation['a']}-{relation['b']} "
                f"iou={relation['iou']:.3f} "
                f"area_ratio={relation['area_ratio']:.3f} "
                f"distance={relation['center_distance']:.1f} "
                f"contains={relation['contains']}"
            )
    ...
    for label in sorted(counts):
        print(f"  {label:<30} {counts[label]}")


def print_temporal_match_diagnostics(captures, evaluated):
    print()
    print("-" * 100)
    print("TEMPORAL MATCH DIAGNOSTICS")
    print("-" * 100)

    capture_sizes = [len(capture) for capture in captures]

    print(
        f"CAPTURE GROUP COUNTS: "
        f"C1={capture_sizes[0]} "
        f"C2={capture_sizes[1]} "
        f"C3={capture_sizes[2]}"
    )

    print(f"TEMPORAL OBJECTS: {len(evaluated)}")

    expected = min(capture_sizes)

    if len(evaluated) < expected:
        print(
            f"MATCH LOSS: {expected - len(evaluated)} "
            f"object(s) below minimum capture count"
        )
    else:
        print("MATCH LOSS: none relative to minimum capture count")

    print(
        "NOTE: this is diagnostic only; "
        "no decision score is changed."
    )
