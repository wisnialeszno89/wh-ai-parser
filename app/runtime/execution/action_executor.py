...
class ActionExecutor:
    ...
    def _remember_workspace(self, vision) -> None:
        ...
        candidate = (bounds.left, bounds.top, bounds.width, bounds.height)
        if current is not None:
            _, _, current_width, current_height = current
            if candidate[2] * candidate[3] < current_width * current_height * 0.80:
                print(
        ...
        print(f"[PLACEMENT] remembered workspace ({bounds.left},{bounds.top},{bounds.width}x{bounds.height})")
    ...
    def _observe_construction_state(self, vision):
        observation = self.construction_state.observe(vision, self.context.gui_state)
        print(
            ...
            f"glass={observation.glass_present} runtime_history={observation.runtime_history_present} "
    ...
    def _guard_create_against_unknown_existing_construction(self, action, vision, start_time):
        ...
        if action.tool == GuiTool.FRAME and self.context.gui_state.frame_point is not None:
            ...
            return ActionResult(False, "FRAME already exists; duplicate CREATE blocked", 1.0, duration_ms)
        if observation.stage == ConstructionStage.EXTERNAL_CONSTRUCTION:
            duration_ms = int((time.perf_counter() - start_time) * 1000)
            print(f"[STATE GUARD] Existing construction detected without runtime creation history; refusing {action.tool.name} CREATE")
            return ActionResult(False, f"External construction detected; {action.tool.name} CREATE blocked until current construction is understood", 0.90, duration_ms)
        return None
    ...
    def _establish_workspace_before_first_tool_click(self) -> None:
        ...
        if self.context.gui_state.workspace_bounds is None:
            raise RuntimeError("Unable to establish construction workspace before tool click")
    ...
    def _resolve_create_point(self, action, vision) -> tuple[int, int]:
        ...
        if action.tool in (GuiTool.SASH, GuiTool.GLASS):
            ...
            if point is None:
                raise RuntimeError(f"{action.tool.name} CREATE requires a valid construction panel")
            return point
        if action.tool == GuiTool.HARDWARE:
            point = self.context.gui_state.sash_point or self.context.gui_state.frame_point or self.context.gui_state.last_created_point
            if point is None:
    ...
    def _workspace_rect(self, vision):
        ...
        if stored is not None:
            ...
            print(f"[PLACEMENT] using workspace anchor ({x},{y},{width}x{height})")
    ...
    def _resolve_panel_point(self, vision, tool) -> tuple[int, int] | None:
        ...
        if mullion is None:
            ...
            point = (canvas.left + canvas.width // 2, canvas.top + canvas.height // 2)
            print(f"[PLACEMENT] single construction cell tool={tool.name} canvas=({canvas.left},{canvas.top},{canvas.width}x{canvas.height}) -> ({point[0]},{point[1]})")
        ...
        if orientation == "horizontal":
            top, bottom = (canvas.top, min(my, canvas.bottom)) if side == "top" else (max(my, canvas.top), canvas.bottom)
            if bottom <= top:
                return None
            x, y = canvas.left + canvas.width // 2, top + (bottom - top) // 2
        else:
            left, right = (canvas.left, min(mx, canvas.right)) if side == "left" else (max(mx, canvas.left), canvas.right)
            if right <= left:
                return None
            x, y = left + (right - left) // 2, canvas.top + canvas.height // 2
        point = (x, y)
        print(f"[PLACEMENT] panel orientation={orientation} side={side} tool={tool.name} canvas=({canvas.left},{canvas.top},{canvas.width}x{canvas.height}) mullion=({mx},{my}) -> ({x},{y})")
        if tool == GuiTool.SASH:
            state.panel_pair_point = point
            state.sash_point = point
            state.last_panel_component = "SASH"
    ...
    def _capture_dynamic_sash_anchor(self) -> None:
        ...
        if window is None:
            ...
            return
        local_point = (int(screen_point[0] - window.left), int(screen_point[1] - window.top))
        self.context.gui_state.sash_point = local_point
        self.context.gui_state.last_selected_point = local_point
        print(f"[SASH] dynamic hardware selection screen={screen_point} origin=({window.left},{window.top}) -> local={local_point}")
    ...
    def _advance_panel_after_glass(self) -> None:
        ...
        print(f"[PLACEMENT] next construction cell side={state.panel_side} orientation={state.mullion_orientation}")
    ...
    def _execute_create(self, action, start_time: float) -> ActionResult:
        ...
        guard_result = self._guard_create_against_unknown_existing_construction(action, vision, start_time)
        ...
        if action.tool in (GuiTool.MULLION, GuiTool.HORIZONTAL_MULLION, GuiTool.MOVABLE_MULLION):
            ...
            self.context.gui_state.mullion_orientation = "horizontal" if action.tool == GuiTool.HORIZONTAL_MULLION else "vertical"
            self.context.gui_state.panel_side = "top" if self.context.gui_state.mullion_orientation == "horizontal" else "left"
            self.context.gui_state.panel_pair_point = None
            self.context.gui_state.sash_point = None
            self.context.gui_state.glass_point = None
