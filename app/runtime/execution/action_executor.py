...
class ActionExecutor:
    ...
    def _execute_create(
        ...
        # stale GUI geometry from before the tool click.
        ...
        placement = self._resolve_create_point(
            action,
            vision,
        )
        ...
        print(
            ...
            f"-> placement local={placement} "
            f"origin={origin}"
        )

        # 4. Place the object at the semantically appropriate location.
        self.click.click_xy(
            placement[0],
            placement[1],
            origin=origin,
        ...
        # A sash is created inside the currently selected frame. Persist the
        # point so subsequent component-selection actions can use the same
        # object context rather than falling back to an arbitrary canvas point.
        if action.tool == GuiTool.SASH:
            self.context.gui_state.last_selected_point = placement
    ...
    def _resolve_create_point(
        self,
        action,
        vision,
    ) -> tuple[int, int]:
        """
        Resolve where a CREATE action should be placed.

        FRAME starts a construction in the movable workspace.
        SASH is a child object and must be created inside the currently
        selected frame, so it reuses the most recent selected point.
        """
        if (
            action.tool == GuiTool.SASH
            and self.context.gui_state.last_selected_point is not None
        ):
            point = self.context.gui_state.last_selected_point

            print(
                f"[CREATE TARGET] SASH inside selected object: {point}"
            )

            return point

        return self.canvas.resolve(
            vision,
        )
